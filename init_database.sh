...
PGPASSWORD='jcf0326_103' psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -f database_postgres.sql
if [ $? -eq 0 ]; then
...
PGPASSWORD='jcf0326_103' psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -f database_weather_data.sql
if [ $? -eq 0 ]; then
...
PGPASSWORD='jcf0326_103' psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -f database_mining_areas.sql
if [ $? -eq 0 ]; then
...
if [ -f "add_indexes.sql" ]; then
    ...
    PGPASSWORD='jcf0326_103' psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -f add_indexes.sql
    if [ $? -eq 0 ]; then
...
if [ -f "optimize_database.sql" ]; then
    ...
    PGPASSWORD='jcf0326_103' psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -f optimize_database.sql
    if [ $? -eq 0 ]; then
...
PGPASSWORD='jcf0326_103' psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -c "
SELECT 
    schemaname,
    tablename,
